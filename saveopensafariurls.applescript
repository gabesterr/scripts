...
<p>" & NameText's text items as text) & "&nbsp;<a href=\"" & subText's text items as text) & "\">" & subText's text items as text) & "</a>"
			set j to j + 1
		end repeat
	else
		set listTextItems to (((listTextItems & "

<p>" & NameItem's text items as text) & "&nbsp;<a href=\"" & TextItem's text items as text) & "\">" & TextItem's text items as text) & "</a>"
	end if
	set i to i + 1
end repeat
